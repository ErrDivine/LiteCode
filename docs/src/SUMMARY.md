# Summary

[Home](README.md)

# Design

- [Product And Runtime Overview](design/overview.md)
- [Architecture](design/architecture.md)
- [Runtime Flows](design/flows.md)
- [Design Contracts](design/contracts.md)

# Rust Workspace

- [Protocol Crate](crates/protocol.md)
- [Session Kernel](crates/session-kernel.md)
- [Scheduler](crates/scheduler.md)
- [OpenAI Client](crates/openai-rs.md)
- [Status Engine](crates/status.md)
- [Persistence Crates](crates/persistence.md)
- [UI Bridge](crates/ui-bridge.md)

# Runtime Entrypoints

- [Binary, CLI, Web, And VSCode Stdio](runtime/entrypoints.md)
- [Local Tool Gateway](runtime/tools.md)

# Applications

- [VSCode Extension](apps/vscode-extension.md)

# Operations

- [Configuration And Data Locations](operations/configuration.md)
- [Build, Test, And Documentation](operations/build-test-docs.md)

# Appendix

- [Module Index](appendix/module-index.md)
- [Interface Reference](appendix/interface-reference.md)
